...
int check_horizontal_line(char *string, int line_length, int x, int compare) {
    ...
    if(string[x] == string[compare])
        ...
        return check_horizontal_line(string, line_length, x - 1, compare + 1) + 1;
}
...
int check_horizontal(char *block, int line_length, int height, int x, int compare, int y) {
    ...
    return check_horizontal(block, line_length, height, x, compare, y + 1) + check_horizontal_line(block + y * (line_length + 1), line_length, x, compare);
}
...
int check_vertical_line(char *block, int height, int line_length, int x, int y, int compare) {
    ...
    if(block[(line_length + 1) * y + x] == block[(line_length + 1) * compare + x])
        ...
        return check_vertical_line(block, height, line_length, x, y - 1, compare + 1) + 1;
}
...
int check_vertical(char *block, int line_length, int height, int y, int compare, int x) {
    ...
    return check_vertical(block, line_length, height, y, compare, x + 1) + check_vertical_line(block, height, line_length, x, y, compare);
}
...
int main(int argc, char *argv[]) {
    ...
    while(block_start < input + size) {
        ...
        for(x = 0; x < line_length; x++) {
            if(check_horizontal(block_start, line_length, height, x, x + 1, 0) == 1)
                break;
        }

        for(y = 0; y < height; y++) {
            if(check_vertical(block_start, line_length, height, y, y + 1, 0) == 1)
                break;
        }
